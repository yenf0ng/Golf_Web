...
<style>
  ...
  .metric-card .label {
    ...
    letter-spacing: 1.5px; color: #8b949e; margin-bottom: 0.25rem;
  }
  .metric-card .value {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 2.2rem; color: #58a6ff; line-height: 1;
  }
  ...
  .media-meta { padding: 0.6rem 0.8rem; font-size: 0.8rem; color: #8b949e; }
  .media-meta strong { color: #e6edf3; }

  .stTabs [role="tab"] {
    font-family: 'Bebas Neue', sans-serif;
    font-size: 1.05rem; letter-spacing: 1.5px; color: #8b949e !important;
  }
...
def render_media(url: str, ext: str):
    if ext in VIDEO_EXTS:
        try:
            ...
            st.markdown(f'<a href="{url}" target="_blank">Open video</a>',
                        unsafe_allow_html=True)
    else:
        st.markdown(
            f'<img src="{url}" style="width:100%;border-radius:6px;display:block;" alt="swing media">',
            unsafe_allow_html=True,
        )
...
with st.sidebar:
    ...
    page = st.radio(
        "", ["Dashboard", "Media Vault", "Log Session", "Upload Media"],
...
def fetch_performance_logs() -> pd.DataFrame:
    res = (supabase.table("performance_logs")
           .select("*").order("session_date", desc=True).execute())
    if res.data:
...
def fetch_video_vault() -> pd.DataFrame:
    res = (supabase.table("video_vault")
           .select("*").order("clip_date", desc=True).execute())
    if res.data:
        df = pd.DataFrame(res.data)
...
if page == "Dashboard":
    ...
    with col_a:
        disp_clubs = st.multiselect(
            "Filter Clubs", options=sorted(df["club_used"].unique()),
    ...
    shape_grp["shot_shape"] = pd.Categorical(
        shape_grp["shot_shape"], categories=SHOT_SHAPES, ordered=True)
    shape_grp = shape_grp.sort_values("shot_shape")

    fig2 = px.bar(shape_grp, x="shot_shape", y="count", color="club_used", barmode="group",
    ...
    fig3 = go.Figure(go.Bar(
        x=cons["club_used"], y=cons["SD"].round(1), marker_color=cons["color"],
        text=cons["SD"].round(1), textposition="outside", textfont_color="#8b949e"))
    fig3.update_layout(**PLOT_LAYOUT, height=340, yaxis_title="Std Dev (yards)", xaxis_title=None,
                       annotations=[dict(
                           text="Green < 10 yds  |  Yellow 10-20 yds  |  Red > 20 yds",
...
# =============================================================
    ...
    st.caption("Videos, GIFs & images of your swings, filterable by club category")
    vdf = fetch_video_vault()

    col_f1, col_f2, col_f3 = st.columns([3, 1, 1])
    with col_f1:
        cat_filter = st.multiselect(
            "Filter by Category", options=CATEGORIES, default=CATEGORIES, key="cat_filter")
    with col_f2:
        ...
        for col, (_, clip) in zip(grid_cols, row.iterrows()):
            ...
            with col:
                ...
                <div class="media-wrapper">
                  <div style="padding:0.6rem 0.8rem 0.3rem;display:flex;gap:6px;align-items:center;">
    ...
    if st.session_state.is_admin:
        st.divider()
        st.markdown("#### Delete Media (Admin)")
        del_opts = {
            f"{r['clip_date'].date()} -- {r['category']} -- {r['type_label']} (id:{r['id']})": r
            for _, r in fdf.iterrows()
        }
        del_label = st.selectbox("Select item to delete", list(del_opts.keys()), key="del_clip")
...
# =============================================================
    ...
    # Shot shape selector -- MUST be outside st.form (st.button forbidden inside forms)
    st.markdown("#### Shot Shape")
    st.caption("Tap a shape below, then fill in the rest and hit Save Shot.")

    shape_icons = {
        "Straight":"->","Draw":"Down-Left","Fade":"Up-Right",
        "Push":"Right","Pull":"Left","Slice":"Curve-Right","Hook":"Curve-Left",
    }
    shape_symbols = {
        "Straight":"straight","Draw":"draw","Fade":"fade",
        "Push":"push","Pull":"pull","Slice":"slice","Hook":"hook",
    }
    ...
    with st.form("log_form", clear_on_submit=True):
        ...
        with f_col2:
            f_total = st.number_input("Total Distance (yards)", min_value=0, max_value=450, step=1, key="f_total")
            f_carry = st.number_input("Carry Distance (yards)", min_value=0, max_value=450, step=1, key="f_carry")

        f_notes = st.text_area("Notes (optional)",
                               placeholder="E.g. Good hip rotation, slight early extension...",
                               key="f_notes")
